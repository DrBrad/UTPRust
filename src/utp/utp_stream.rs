...
impl UtpStream {
    ...
    async fn receiver(&mut self) {
        let mut buf = [0u8; 65535];

        loop {
            let (size, src_addr) = {
                self.socket.recv_from(&mut buf).expect("Failed to receive message")
            };
            ...
            match packet.header._type {
                UtpType::Data => {
                    self.receive_buffer.append(&mut packet.payload.unwrap());
                }
                _ => {
                }
            }
